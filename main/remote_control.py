...
# Rocky's BLE Service and Characteristic UUIDs
...
def print_menu():
    ...
    print("  ai: AI Autopilot Mode (telemetry-driven)")
...
async def ai_autopilot_mode(client):
    if msvcrt is None:
        print("[-] AI mode requires Windows OS.")
        await asyncio.sleep(2)
        return

    clear_screen()
    print("=" * 60)
    print("                 AI AUTOPILOT MODE ACTIVE")
    print("=" * 60)
    print("  Telemetry-driven behavior loop")
    print("    [q] or [ESC] Exit AI mode")
    print("=" * 60)
    print("Listening for telemetry and reacting...")

    last_touch_pressed = (sensor_touch == "Pressed")
    last_proximity = sensor_proximity
    last_action_time = 0.0
    next_wander_time = time.monotonic() + random.uniform(2.5, 4.5)

    while True:
        try:
            key = await asyncio.wait_for(get_key_async(), timeout=0.05)
        ...
        now = time.monotonic()
        touch_pressed = (sensor_touch == "Pressed")
        proximity = sensor_proximity

        if proximity == "Lifted":
            if last_proximity != "Lifted" or (now - last_action_time) > 2.0:
                await send_command(client, "mood_default", silent=True)
                await send_command(client, "home", silent=True)
                last_action_time = now
                next_wander_time = now + random.uniform(2.5, 4.5)
        elif touch_pressed and not last_touch_pressed:
            await send_command(client, "mood_happy", silent=True)
            if random.random() < 0.5:
                await send_command(client, "bounce", silent=True)
            else:
                await send_command(client, "dance", silent=True)
            last_action_time = now
            next_wander_time = now + random.uniform(3.0, 6.0)
        elif now >= next_wander_time and (now - last_action_time) > 1.0:
            await send_command(client, "mood_default", silent=True)
            action_pool = [
                "tilt",
                "tilt",
            ...
            await send_command(client, random.choice(action_pool), silent=True)
            last_action_time = now
            next_wander_time = now + random.uniform(3.5, 7.5)
...
async def composer_mode(client):
    ...
    print("=" * 60)
    print("  Play notes interactively on Rocky's buzzer:")
    ...
    try:
        ...
        while True:
            ...
            if user_input == 'q':
                ...
                await ai_autopilot_mode(client)
                continue
