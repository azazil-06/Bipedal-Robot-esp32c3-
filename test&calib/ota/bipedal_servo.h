...
// Set true while servos are moving (main.ino skips OLED I2C during this window)
extern volatile bool g_servoMotionActive;
...
public:
    ...
    void begin(int pinLH, int pinRH, int pinLF, int pinRF) {
        ...
        // Home is deferred until user powers on (see main.ino ensureLegsHomed)
    }
    ...
    /**
     * Defensive pose: feet together, slight crouch (proximity / angry hold).
     */
    void closeLegs() {
        moveServosScaled(100, 90, 75, 105, 500);
        moveServosScaled(100, 90, 70, 110, 400);
    }
    ...
    /**
     * Slide/shuffle sideways without lifting feet high.
     */
    void slideLeft(int steps = 1) {
        for (int i = 0; i < steps; i++) {
            moveServosScaled(100, 90, 70, 110, 220);
            moveServosScaled(55, 90, 70, 110, 280);
            moveServosScaled(100, 90, 90, 90, 220);
        }
        home();
    }

    void slideRight(int steps = 1) {
        for (int i = 0; i < steps; i++) {
            moveServosScaled(100, 90, 110, 70, 220);
            moveServosScaled(145, 90, 110, 70, 280);
            moveServosScaled(100, 90, 90, 90, 220);
        }
        home();
    }
    ...
    /**
     * Small in-place shuffle (weight shift side to side).
     */
    void shuffle() {
        for (int i = 0; i < 2; i++) {
            moveServosScaled(100, 90, 120, 60, 200);
            moveServosScaled(100, 90, 60, 120, 200);
        }
        home();
    }

    /**
     * Low moonwalk-style drag (one foot slides back).
     */
    void moonwalkStep() {
        moveServosScaled(100, 90, 160, 135, 250);
        moveServosScaled(100, 135, 160, 80, 350);
        moveServosScaled(100, 90, 90, 90, 250);
        moveServosScaled(100, 90, 45, 20, 250);
        moveServosScaled(145, 45, 45, 100, 350);
        moveServosScaled(100, 90, 90, 90, 250);
        home();
    }
};
